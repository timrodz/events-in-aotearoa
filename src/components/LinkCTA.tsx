...
export const LinkCTA = ({ label, href, classNames }: Props) => {
  return (
    <Link
      ...
      passHref
      className={cx([
        "group relative inline-flex w-fit cursor-pointer items-center justify-between overflow-hidden rounded-full border border-white/10 bg-white/10 py-[3px] pr-[3px] pl-2 text-base font-medium backdrop-blur-xs transition-all hover:bg-transparent md:py-1 md:pr-1 md:pl-3",
        classNames,
      ])}
    >
      <span className="z-10 px-3 text-white transition-colors duration-300 group-hover:text-lime-200">
        {label}
      </span>
