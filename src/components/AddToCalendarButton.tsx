...
const CalendarLink = ({
  ...
  return (
    <a
      href={url}
      target="_blank"
      className="flex items-center gap-2 text-zinc-800 hover:bg-zinc-50 px-4 py-2 whitespace-nowrap"
    >
      {children}
      {name}
    </a>
    ...
    <div className="flex items-center justify-center z-0">
      <span ref={buttonRef} className="inline-block relative">
        <button onClick={() => setIsOpen(!isOpen)}>
          <CalendarPlusIcon color="#7fcc03" />
        </button>
        <ul
          className={`${isOpen ? "opacity-100" : "opacity-0 invisible"} absolute top-full -left-20 min-w-full border shadow-lg py-2 px-1 transition-opacity bg-white z-100`}
        >
          <li>
